...
This document covers the detailed system design, design decisions, and schemas behind the [Spotify Trending Songs pipeline](../README.md). See the README for Quick Start and how to run it.
...
### Lessons Learned
A few issues surfaced during development and were fixed:
- **Trino `MERGE INTO` syntax differences** — Trino's Iceberg connector doesn't accept the same `MERGE INTO` syntax as Spark SQL; the batch DAG's upsert had to be rewritten to match Trino's supported grammar.
- **Missing Docker Compose healthchecks** — several services had no `healthcheck`, so dependent services could start before Kafka/ClickHouse/Trino/MinIO were actually ready to accept connections. Healthchecks with `condition: service_healthy` were added across the stack to fix startup-order races.
- **Missing `non_replicated_deduplication_window`** — ClickHouse's insert deduplication only works within a bounded window of recent inserted blocks; without explicitly setting `non_replicated_deduplication_window` on `play_facts`, retried inserts outside the default window weren't deduplicated.
...
```
Idempotent writes via `MERGE INTO ... ON target.event_id = source.event_id WHEN NOT MATCHED THEN INSERT`.

### play_facts_historical (Silver, Permanent)
...
```
Upserted daily via `MERGE INTO` keyed on `session_id`.
